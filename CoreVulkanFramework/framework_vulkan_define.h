...
#define RZVK_CLEAR_DEPTH FmtVector2T<float>(1.0f, 0.0f)
	...
	// rz_vk framework operation_phase.
	enum RZVK_CONFIG_OPERATE_MODE {
		OPER_CONFIG_ONLY    = 1 << 0, // 无处理链,单个.
		OPER_CONFIG_STORAGE = 1 << 1, // Subpass位于存储(显示)段.
		OPER_CONFIG_BEGIN   = 1 << 2, // Subpass位于处理链开始段.
		OPER_CONFIG_MIDDLE  = 1 << 3  // Subpass位于处理链中间段. 
	};

	// rz_vk framework subpass_dependency.
	enum RZVK_CONFIG_DEPEND_MODE {
		DEPEND_CONFIG_ONLY         = 1 << 0,
		DEPEND_CONFIG_COLOR_A_TO_B = 1 << 1,
		DEPEND_CONFIG_COLOR_OUT    = 1 << 2
	};
	...
	RZ_VULKAN_DEFSTD struct VulkanTextureColor {
		...
		// color_texture create format.
		VkFormat ColorTextureFormat = VK_FORMAT_UNDEFINED;
	};
