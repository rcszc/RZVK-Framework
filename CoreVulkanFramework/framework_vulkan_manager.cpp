...
namespace RZ_VULKAN {
	namespace RenderPass {
		...
		void RenderPassManger::CreateDepthAttachment(VkFormat dep_format, const FmtVector2T<float>& clear) {
			...
			DepthAttachment.format  = dep_format;
			...
			DatasetAttachments.push_back(DepthAttachment);
		...
		size_t RenderPassManger::PushCreateAttachment(
			VkFormat img_format, RZVK_CONFIG_OPERATE_MODE oper, const FmtVector4T<float>& clear
		) {
		...
		size_t RenderPassManger::PushCreateSubRenderPass(size_t attchment_index) {
			...
			DatasetAttachmentsRef[AttachmentsRefCount] = AttachmentReference;
			...
			SubRenderPass.pColorAttachments    = &DatasetAttachmentsRef[AttachmentsRefCount];
			// count attachment, clamp count_value.
			++AttachmentsRefCount;
			AttachmentsRefCount = AttachmentsRefCount >= PassAttachmentsMax ? PassAttachmentsMax - 1 : AttachmentsRefCount;
		...
		size_t RenderPassManger::PushCreateSubDependency(const FmtVector2T<size_t>& subpass, RZVK_CONFIG_DEPEND_MODE oper) {
			...
			SubPassDependency.srcSubpass = (uint32_t)subpass.vector_x;
			SubPassDependency.dstSubpass = (uint32_t)subpass.vector_y;
			
			switch (oper) {
			case(DEPEND_CONFIG_ONLY): {
			...
			case(DEPEND_CONFIG_COLOR_A_TO_B): {
	...
	namespace RenderCommand {
		...
		void RenderCommandManager::CommandNextSubpass() {
			vkCmdNextSubpass(VulkanCommandBuffer, VK_SUBPASS_CONTENTS_INLINE);
		}
